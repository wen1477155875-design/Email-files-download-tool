...
def _make_mail(entry_id, sender, subject, received_utc, attachments):
    mail = MessageInfo(entry_id=entry_id, subject=subject, sender=sender,
                       received_utc=received_utc, item=entry_id)
...
""", encoding="utf-8")
    ...
    checks = [
        ("下载 3 个附件（跳过内嵌图 + 非目标发件人 + 非当天邮件）", first["downloaded"] == 3),
        ("没有失败项", first["failed"] == 0),
        ("zip 已解压", first["extracted"] == 1),
        ("解压出 2 个文件（含子目录内的）", first["extract_files"] == 2),
        ("第 2 次运行 0 新增下载", second["downloaded"] == 0),
        ("第 2 次运行 3 个命中去重", second["skipped_already"] == 3),
        ("关闭 only_today 后补下前天邮件 + 丢失文件重下", third["downloaded"] == 2),
        ("补下时未丢的附件不重复下载", third["skipped_already"] == 2),
