...
def load_values(path: Path = CONFIG_PATH) -> dict:
    ...
    return {
        ...
        "schedule_mode": str(schedule.get("mode", "daily")),
...
def save_values(values: dict, path: Path = CONFIG_PATH) -> None:
    ...
    text = _update_section(text, "schedule", {
        "mode": str(values["schedule_mode"]),
        "time": _quote(values["daily_time"]),
        "interval_minutes": str(int(values["interval_minutes"])),
    })
...
def validate(values: dict) -> str:
    ...
    if values["schedule_mode"] == "daily" and not TIME_RE.match(str(values["daily_time"]).strip()):
        return "每日运行时间格式应为 HH:MM，例如 10:00"
    if values["schedule_mode"] == "minutes" and not 1 <= int(values["interval_minutes"]) <= 1440:
        return "间隔分钟应在 1~1440 之间"
    return None
...
class App:
    ...
    def _build_ui(self):
        ...
        # -- 定时 --
        f5 = ttk.LabelFrame(outer, text="定时方式")
        f5.pack(fill="x", **pad)
        ttk.Radiobutton(f5, text="每天", value="daily", variable=self.var_sched_mode,
                        command=self._sync_sched).grid(row=0, column=0, **pad)
        self.ent_time = ttk.Entry(f5, textvariable=self.var_daily_time, width=8)
        self.ent_time.grid(row=0, column=1, sticky="w", **pad)
        ttk.Label(f5, text="运行 (HH:MM)").grid(row=0, column=2, sticky="w")
        ttk.Radiobutton(f5, text="每隔", value="minutes", variable=self.var_sched_mode,
                        command=self._sync_sched).grid(row=0, column=3, **pad)
        self.spn_interval = ttk.Spinbox(f5, from_=1, to=1440, textvariable=self.var_interval, width=6)
        self.spn_interval.grid(row=0, column=4)
        ttk.Label(f5, text="分钟运行一次").grid(row=0, column=5, sticky="w")
        self._sync_sched()
    ...
    def _sync_sched(self):
        is_daily = self.var_sched_mode.get() == "daily"
        self.ent_time.configure(state="normal" if is_daily else "disabled")
        self.spn_interval.configure(state="disabled" if is_daily else "normal")
    ...
    def _collect(self) -> dict:
        ...
        return {
            ...
            "schedule_mode": self.var_sched_mode.get(),
            "daily_time": self.var_daily_time.get().strip(),
            "interval_minutes": int(self.var_interval.get() or 30),
        }
    ...
    def load_into_ui(self):
        ...
        self.var_sched_mode.set(v["schedule_mode"])
        self.var_daily_time.set(v["daily_time"])
        self.var_interval.set(v["interval_minutes"])
        self._sync_sched()
    ...
    def on_run_now(self):
        ...
        self._log(f"开始运行（dry_run={dry}" + (f"，指定日期 {date_text}" if date_text else "") + "）…")
